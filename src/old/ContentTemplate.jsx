  ...
  useEffect(() => {
	async function getPageData() {
		...
		const tparArray = docData.Content.map((data) => 
			{
				var strArr = data.split('/end ')
				if (strArr[1]) {
					var el = (data.slice(0,2) == 'h1') ? <p className= {`header-text bg-gradient-to-r ${colorData[props.mainColor]} ${strArr[1]} to-transparent border-[#000000]/10 max-w-[400px] flex items-center h-[40px] dark:text-shadow-drop`} key= {docData.Content.indexOf(data)}> {strArr[0].slice(2, strArr[0].length)}</p> : React.createElement('p',{className: 'par-text ' + strArr[1], key: docData.Content.indexOf(data)}, strArr[0])
				} else {
					var el = (data.slice(0,2) == 'h1') ? <p className= {`header-text bg-gradient-to-r ${colorData[props.mainColor]} to-transparent border-[#000000]/10 max-w-[400px] flex items-center h-[40px] dark:text-shadow-drop`} key= {docData.Content.indexOf(data)}>{data.slice(2, data.length)}</p> : <p className= {'par-text'} key={docData.Content.indexOf(data)}>{data}</p>
				}
				return (el)
			}
		  );
